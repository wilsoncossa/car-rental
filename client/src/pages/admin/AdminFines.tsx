...
export default function AdminFines() {
  ...
  const [newFine, setNewFine] = useState({ userId: '', description: '', amount: '', bookingId: '' });
  ...
  const createFine = useMutation({
    ...
    onSuccess: () => {
      ...
      setOpen(false);
      setNewFine({ userId: '', description: '', amount: '', bookingId: '' });
    },
  });

  if (isLoading) {
    return (
      <DashboardLayout title="Gestão de Multas">
  ...
  return (
    <DashboardLayout title="Gestão de Multas" subtitle="Registar e gerir multas de clientes">
      <div className="flex justify-end mb-4">
        <Dialog open={open} onOpenChange={setOpen}>
          ...
          <DialogContent>
            <DialogHeader>
              <DialogTitle>Registar Multa</DialogTitle>
            </DialogHeader>
            <div className="space-y-4 pt-4">
              <div>
                <Label>Cliente</Label>
                <Select value={newFine.userId} onValueChange={(v) => setNewFine(p => ({ ...p, userId: v }))}>
                  <SelectTrigger data-testid="select-fine-user">
              ...
              <Button
                className="w-full"
                onClick={() => createFine.mutate({
                  userId: newFine.userId,
                  description: newFine.description,
                  amount: Number(newFine.amount),
                  bookingId: newFine.bookingId ? Number(newFine.bookingId) : null,
                  status: 'pending',
                })}
                disabled={!newFine.userId || !newFine.description || !newFine.amount || createFine.isPending}
                data-testid="button-submit-fine"
              >
                {createFine.isPending ? <Loader2 className="h-4 w-4 animate-spin mr-2" /> : null}
                Registar Multa
              </Button>
            </div>
          </DialogContent>
      ...
      <Card>
        <CardHeader>
          <CardTitle>Multas ({finesList.length})</CardTitle>
        </CardHeader>
        <CardContent>
          <div className="space-y-4">
              ...
              <div key={fine.id} className="flex items-center justify-between border-b pb-3 last:border-0" data-testid={`card-fine-${fine.id}`}>
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">
                    {fine.createdAt ? format(new Date(fine.createdAt), "dd/MM/yyyy") : '—'}
                  </p>
                </div>
            ...
            {finesList.length === 0 && (
              <div className="text-center py-8 text-muted-foreground">Nenhuma multa registada.</div>
            )}
          </div>
        </CardContent>
      </Card>
