...
export default function AdminBookings() {
  ...
  if (isLoading) {
    return (
      <DashboardLayout title="Gestão de Reservas">
  ...
  return (
    <DashboardLayout title="Gestão de Reservas" subtitle="Todas as reservas do sistema">
      <Card>
        <CardHeader>
          <CardTitle>Reservas ({allBookings.length})</CardTitle>
        </CardHeader>
        <CardContent>
            ...
            {allBookings.map((booking: any) => (
              <div key={booking.id} className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b pb-4 last:border-0" data-testid={`card-booking-${booking.id}`}>
                <div className="space-y-1">
                  ...
                  <p className="text-xs text-muted-foreground">Pagamento: {booking.paymentMethod?.toUpperCase()}</p>
                </div>
